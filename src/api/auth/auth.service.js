...
export const authTerminal = async (terminalNumber, serialNumber) => {
	...
	if (!terminal) throw new ApiError(apiCode.NOT_FOUND, `The terminal does not exist`)
	let user = await Users.getUserBySn(serialNumber);
	if (!user) throw new ApiError(apiCode.NOT_FOUND, `The user does not exist`)
	let grant = await Grants.getGrant(user.id, terminal.id);
	if (!grant) throw new ApiError(apiCode.FORBIDDEN, `The user(${user.id}) does not have grant for terminal(${terminal.id})`);
